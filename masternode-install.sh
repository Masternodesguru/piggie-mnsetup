...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='piggie.conf'
CONFIGFOLDER='/root/.piggie'
COIN_DAEMON='piggied'
COIN_CLI='piggie-cli'
COIN_TX='piggie-tx'
COIN_PATH='/usr/local/bin/'
OS_VERSION=$(lsb_release -d)
COIN_TGZP='https://github.com/masternodeguru/piggie-mnsetup/releases/download/1001/linux-daemon.zip'
COIN_BOOTSTRAP=''
COIN_BOOTSTRAP_NAME=$(echo $COIN_BOOTSTRAP | awk -F'/' '{print $NF}')
COIN_TGZ=$(echo $COIN_TGZP | awk -F'/' '{print $NF}')
COIN_NAME='piggie'
COIN_NAME_OLD='piggied'
PROJECT_NAME='piggie (PIGGIE)'
COIN_EXPLORER='http://217.69.7.182'
COIN_PORT=22190
RPC_PORT=22189
...
txindex=1
staking=1
addnode=78.141.201.151
addnode=216.238.71.44
addnode=45.32.235.233
EOF
}
...
cat << EOF > /root/piggie-general-info
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
echo -e "${GREEN}$PROJECT_NAME General Info: ${NC}"
$COIN_CLI getinfo
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/piggie-general-info

cat << EOF > /root/piggie-fee-info
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
echo -e "${GREEN}$PROJECT_NAME Fee Info: ${NC}"
$COIN_CLI getfeeinfo 100
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/piggie-fee-info

cat << EOF > /root/piggie-networkinfo
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
echo -e "${GREEN}$PROJECT_NAME Network Info: ${NC}"
$COIN_CLI getnetworkinfo
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/piggie-networkinfo

cat << EOF > /root/piggie-masternode-stats
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
...
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/piggie-masternode-stats

cp /root/.bashrc /root/.bashrc.backup
sed '/feestats/d' /root/.bashrc | sed '/networkstats/d' | sed '/mnstats/d' | sed '/piggieinfo/d' > /root/tmp
mv /root/tmp /root/.bashrc
echo -e "alias piggieinfo='/root/piggie-general-info'" >> /root/.bashrc
echo -e "alias feestats='/root/piggie-fee-info'" >> /root/.bashrc
echo -e "alias networkstats='/root/piggie-networkinfo'" >> /root/.bashrc
echo -e "alias mnstats='/root/piggie-masternode-stats'" >> /root/.bashrc
exec bash
}
...
function important_information() {
 ...
 echo -e "You can also use ${RED}piggie-cli getinfo${NC}, ${RED}piggie-cli getnetworkinfo${NC}, ${RED}piggie-cli getconnectioncount${NC}commands for a nice looking infos.${NC}"
 echo -e "${BLUE}================================================================================================================================${NC}"
